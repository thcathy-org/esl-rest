...
class LocalAiServiceTest {
    ...
    void isConfigured_returnsFalseWhenUrlMissing() {
        var service = new LocalAiService("", "api-key", "kokoro", "af_heart", 120);
        assertFalse(service.isConfigured());
    }

    @Test
    void isConfigured_returnsFalseWhenApiKeyMissing() {
        var service = new LocalAiService("https://local-ai.example", "", "kokoro", "af_heart", 120);
        assertFalse(service.isConfigured());
    }

    @Test
    void isConfigured_returnsTrueWhenBothSet() {
        var service = new LocalAiService("https://local-ai.example", "api-key", "kokoro", "af_heart", 120);
        assertTrue(service.isConfigured());
    }

    @Test
    void isConfigured_addsHttpsSchemeWhenMissing() {
        var service = new LocalAiService("local-ai.example", "api-key", "kokoro", "af_heart", 120);
        assertTrue(service.isConfigured());
    }

    @Test
    void textToSpeech_throwsWhenNotConfigured() {
        var service = new LocalAiService("", "", "kokoro", "af_heart", 120);
